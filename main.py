...
def length(twit):
    ...
    for i in range(len(twit)):
        word_twit = twit[i].split(" ")
        if length_twit.count(len(word_twit)) == 0:
            length_twit.append(len(word_twit))
            number.append(1)
        else:
            number[length_twit.index(len(word_twit))] += 1
    fr = []
    for i in range(len(length_twit)):
        fr.append([length_twit[i], number[i]])
    fr = sorted(fr, key=lambda x: int(x[1]), reverse=True)
    return fr
...
def main():
    ...
    print(len(tokens))
    f.close()
    length_word = length(twits)
    f4 = open('twits_length.txt', 'a', encoding='utf-8')
    for i in range(len(length_word)):
        f4.write('{} - {} - {}%\n'.format(length_word[i][0], length_word[i][1], float(length_word[i][1]) / len(twits)))
    f4.close()
    time_rules(twits)
    rules(twits)
    ...
    for i in range(len(freq)):
        f3.write('{} - {} - {}%\n'.format(freq[i][0], freq[i][1], float(freq[i][1])/len(tokens)))
    f3.close()
    adjectives(freq)
